...
CONTAINER_ID=$(docker ps -q --filter ancestor=cse138asgn2)
...
if [ ! -z "$CONTAINER_ID" ]; then
  ...
  echo "No running container found for cse138asgn2."
fi

# Step 2: Build the Docker image
echo "Building the Docker image..."
docker build -t cse138asgn2 .
...
docker run --rm -p 8090:8090 cse138asgn2
